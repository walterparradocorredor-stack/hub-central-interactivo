...
import {
  Sparkles, Cpu, CheckCircle2, ArrowRight, ShieldCheck, Zap, Laptop, ShoppingCart, Bot, CreditCard, FileText, MapPin, Lock, Send, Layers, HelpCircle, Check, ArrowLeft, RefreshCw, Smartphone, ExternalLink, Monitor
} from "lucide-react";

export default function CotizarWebPage() {
  const [activeView, setActiveView] = useState<"cotizador" | "simulador">("cotizador");
...
Hola Dr. Walther Parrado / J&M Tech Solutions, he generado la estructura técnica de mi proyecto en el portal. ¡Me gustaría recibir la cotización formal y agendar una reunión ejecutiva!`;

    return `https://wa.me/573045788873?text=${encodeURIComponent(msg)}`;
  };

  return (
    <main className="min-h-screen bg-[#04060a] text-white p-4 md:p-8 font-sans relative overflow-hidden selection:bg-purple-600 selection:text-white pt-24 pb-16">
      {/* GLOW ORBS */}
      ...
      <div className="max-w-6xl mx-auto space-y-6 relative z-10">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-black tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-purple-200 to-cyan-300">
                AI Web Architect & Simulador Responsivo
              </h1>
              <p className="text-xs text-gray-400">
                Diseña tu proyecto web o prueba sitios en tiempo real en mockups de celulares.
              </p>
            </div>
          </div>

          {/* DUAL VIEW SELECTOR BUTTONS */}
          <div className="flex items-center gap-1.5 bg-slate-900/90 p-1.5 rounded-2xl border border-white/15 backdrop-blur-xl shadow-xl">
            <button
              onClick={() => setActiveView("cotizador")}
              className="px-4 py-2.5 rounded-xl font-extrabold text-xs transition-all flex items-center gap-2 cursor-pointer bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg shadow-purple-500/30"
            >
              <Sparkles className="w-4 h-4 text-cyan-400" /> 🧙‍♂️ Cotizador Web & SaaS
            </button>
            <Link
              href="/simulador"
              className="px-4 py-2.5 rounded-xl font-extrabold text-xs transition-all flex items-center gap-2 cursor-pointer text-cyan-300 hover:text-white bg-cyan-500/20 hover:bg-cyan-500/30 border border-cyan-500/40"
            >
              <Smartphone className="w-4 h-4 text-cyan-400" /> 📱 Abrir Simulador Móvil Fullscreen
            </Link>
          </div>
        </header>

        {/* VIEW 1: COTIZADOR WEB & SAAS */}
                ...
                <div className="bg-slate-900/60 border border-white/10 p-6 rounded-3xl space-y-4">
                  ...
                  <div className="flex items-start gap-3 bg-slate-950 p-4 rounded-xl border border-white/5">
                    ...
                    <label htmlFor="habeas" className="text-xs text-gray-300 leading-relaxed cursor-pointer">
                      Autorizo el tratamiento de mis datos personales de acuerdo con la <strong>Ley 1581 de 2012 (Habeas Data)</strong> para recibir la propuesta comercial y cotización ejecutiva del Dr. Walther Parrado.
                    </label>
                  </div>
                </div>
        ...
        {/* VIEW 2: SIMULADOR DE CELULARES & DISPOSITIVOS MÓVILES */}
        {activeView === "simulador" && (
          <div className="space-y-4">
            <div className="bg-slate-900/80 border border-white/10 p-4 px-6 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-3 text-xs">
              <div>
                <h3 className="font-bold text-white text-sm flex items-center gap-2">
                  J&M ResponsiveLab — Simulador en Tiempo Real 📱
                </h3>
                <p className="text-gray-400 text-[11px]">
                  Prueba la responsividad de sitios web en mockups premium de iPhone, Android, iPad y Pantallas POS.
                </p>
              </div>
              <a
                href="https://www.jymtechsolutions.online/device-simulator/index.html"
                target="_blank"
                rel="noopener noreferrer"
                className="px-4 py-2 rounded-xl bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 font-bold text-xs border border-cyan-500/40 flex items-center gap-1.5 self-start md:self-auto"
              >
            ...
            {/* FULL HEIGHT CONTAINER FOR SIMULATOR GUIDE (min-h-[880px] to prevent cutoff) */}
            <div className="w-full h-[880px] rounded-3xl overflow-hidden border border-white/15 bg-slate-950 shadow-2xl relative">
              <iframe
                src="https://www.jymtechsolutions.online/device-simulator/index.html"
                title="J&M ResponsiveLab - Simulador de Dispositivos Online"
